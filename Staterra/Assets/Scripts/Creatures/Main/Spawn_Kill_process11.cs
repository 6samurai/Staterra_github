...
public class Spawn_Kill_process11 : MonoBehaviour {
...
//	int[] death = new int[17];
	...
	//	GameObject cntrl = GameObject.Find("Animal_Object_control");
		//Spawn_Kill_processv10 tier = cntrl.GetComponent<Spawn_Kill_processv10>();
		...
		//	for (int i =0; i <15; i++) {
			...
			//	Debug.Log("objects in array " + copepod_class.alive[1]);
				...
				if ( crab_class.pop != 0) {
					...
					Debug.Log("death");
					Debug.Log("objects in array " + crab_class.alive[0]);
					
					crab_class.Death_Cycle (0);
